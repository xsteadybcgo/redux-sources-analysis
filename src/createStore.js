...
export default function createStore(reducer, preloadedState, enhancer) {
  ...
  let isDispatching = false; // dispatch标识位，reducer调用错误的时候会设为false
   ...
   */
  // 初始化时候，currentState===preloadedState, 即为 createStore 传入的state
  function getState() {
    return currentState; // 初始化的时候，
  }
   ...
   *
   * dispatch action时，listener将被执行；在执行listener过程中来订阅或取消订阅不影响dispatch的执行，即已存在的listerner执行
   * 但是下一次dispatch会受到影响，因为下一次的subscriptions快照已经变化
   *
   ...
   * 保证`dispatch()`开始之前，所有注册的subscribe订阅将在它退出时，使用最新的state来调用
   *
   ...
   * will be notified.
   *
   * reducer函数通过createStore执行的返回值被视为next state tree
   *
   ...
   *
   * 逐一执行 nextlisteners, 返回dispatch的action
   *
